...
- `npm test` — all harnesses; pure Node, no test framework. Individually: `node test/loader.test.js` (Phase 1: loader/resolver), `node test/store.test.js` (Phase 2: store/watchers), `node test/render.test.js` (Phase 3: the Node-testable renderer slice), `node test/wire.test.js` (Phase 4: the Wire, DB-free), `node test/auth.test.js` (Phase 5: identity, DB-free). Expected outcomes per fixture are recorded in `test/fixtures/README.md`.
