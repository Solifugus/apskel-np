...
- `node tools/poke.js <storePath> --email <e> --password <pw> [--set "v"]` — acceptance criterion 6: reads and sets a bound field through the Wire (`apskel.data.get`/`.set`, base revision handled).
